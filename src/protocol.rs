...
/// Encrypt a client DNS query with a proper config,
/// wrap the query with one additional layer of encryption per proxy,
/// return the encrypted query and client secret.
pub fn encrypt_query_with_proxies<R: RngCore + CryptoRng>(
    query: &ObliviousDoHMessagePlaintext,
    target_url: &String,
    config: &ObliviousDoHConfigContents,
    ...
    for i in 0..proxy_keys.len() {
        ...
        // Add target url to front of the message for proxy 0
        // Add url to the next proxy for all other proxies
        let mut next_url: Vec<u8> = if i == 0 { target_url.as_bytes().to_vec() } else { proxy_names[i - 1].as_bytes().to_vec() };
        assert!(next_url.len() <= next_url.len());
        next_url.insert(0, next_url.len().try_into().unwrap());
        next_url.extend(vec![0u8; MAX_URL_LENGTH - next_url.len()]);

        let msg: Bytes = [next_url, key_id.to_vec(), result].concat().into();
