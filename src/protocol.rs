...
pub fn encrypt_query_with_proxies<R: RngCore + CryptoRng>(
    ...
    for count in 0..num_hops {
        ...
        // concat_msg = [ p | url_len | url | key_id | msg_len | msg | dummy ], where
        //   p (1B): 1 if the next hop is a proxy, 0 if target
        //   url_len(1B): length of the next hop url
        //   url(?B): the next hop url
        //   key_id(?B): public key of the target
        ...
        let dummy_len = (rng.next_u32() % 32) as usize;
